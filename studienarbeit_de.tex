...
der Vorderachse des Fahrzeugs zum Pfad. Die geometrische Beziehungen zwischen
den Gr"o{\ss}en ist in Abb. \ref{stanley} zu sehen.
...
gelenkter Vorderachse \cite{car-model}. Dieses Modell wird durch die folgende
nichtlineare Zustandsraumdarstellung dargestellt: 
...
zwischen dem Hinterradachse und dem Vorderradachse.

Der Lenkwinkel $\varphi$ ist selbst ein dynamisches System, das experimentell
gezeigt wird, ein PT1-Glied zu verhalten.
Der PT1-Glied wird durch die folgende Gleichung modelliert:\begin{equation}
        \dot{\varphi} = \frac{\left(u-\varphi\right)}{T},
\end{equation}
womit der Zeitkonstante $T$ konfiguierbar ist. Das Verhalten des Stanley-Reglers bei dynamischen Einflüssen auf den Lenkwinkel
...
dieser bestimmt, indem der Punkt mit dem geringsten Abstand $\lambda$ zum
Vorderachsmittelpunkt gefunden wird. An diesem Punkt wird
...
\begin{figure}[h]
    ...
    Parameter. $\underline{p}_P(s^*)$ ist hier der Pfadpositionsvektor mit dem
...
\begin{align}
    \underline{\nu} &:= \begin{bmatrix} \cos(\theta) & \sin(\theta) \end{bmatrix}^T, \\
        \underline{\nu}_{\perp} &:= \begin{bmatrix} \cos(\theta + \pi/2) & \sin(\theta + \pi/2) \end{bmatrix}^T, \\
            \Delta\underline{x} &:= \underline{p}_V - \underline{p}_P(s), \\
    s^* &:= \argmin  || \Delta\underline{x} ||_2, \\
    \underline{\lambda} &:= \underline{p}_P(s^*), \\
    \theta_d &:= \arctan\left(\frac{\frac{d}{dy}\underline{\lambda}}{\frac{d}{dx}\underline{\lambda}}\right), \\
...
\hline
\end{tabular}
    \caption{Minimalen und Maximalen Werte f"ur die Bestimmung des gelben Farbtonbereiches.}
    \label{hsvtable}
\end{center}
\end{table}
...
Spitzenwert des Histogramms aus des vorherigen Schritts gesetzt. Dann wird die
...
Polynoms am unteren Rand des Bildes, d.h. bei $y=h$, $\sigma_\mathrm{mpp}$ eine
Skalierungskonstante zur Umrechnung von Pixeln in Meter und $e_{V}$ der Abstand
des Fahrzeugs zur Fahrspur. Zusammenfassend wird die Differenz aus der
$x$-Komponente des Mittelpunkts und der $x$-Komponente des Polynompunkts am
unteren Bildrand in Meter umgerechnet.
...
Die Ergebnisse des Experiments ist in Tab. \ref{fehlerwinkel} zu sehen.
...
\hline
\end{tabular}
    \caption{Maximal m"oglicher Fehlerwinkel beider Regler, wenn die im oder gegen den Uhrzeigersinn
    gedreht wird.}
    \label{fehlerwinkel}
\end{center}
\end{table}
...
Abb. \ref{ab:1.0} und \ref{ab:1.25} zeigen, dass bei niedrigeren
Vorw"artsgeschwindigkeiten verhalten die beide Regler "ahnlich. Aber Abb.
...
Die Ergebnisse des Experiments ist in Tab. \ref{reglerausgabe} zu sehen.
...
durchschnittliche Reglerausgang entsprechend an. Dies ist plausibel, da eine
größere Vorwärtsgeschwindigkeit eine schnellere Reaktion des Reglers nötig
macht. Der Unterschied zwischen den beiden Reglern liegt jedoch im zeitlichen
Verlauf der beiden Signale. Bei Vorwärtsgeschwindigkeiten zwischen 1,0 und 1,5
$\frac{\mathrm{m}}{\mathrm{s}}$ ist der Signalverlauf der beiden Regler
ähnlich, wie in Abb. \ref{reg:1.0} und \ref{reg:1.5} zu sehen sind. Bei 1,75
und 2,0 $\frac{\mathrm{m}}{\mathrm{s}}$ verschlechtert sich jedoch der
...
\begin{center}
        \begin{tabular}{|c|c|c|}
            ...
            \hline
        \end{tabular}
        \caption{Zeitlichen Mittelwerte der Reglerausg"ange bei den verschiedenen
